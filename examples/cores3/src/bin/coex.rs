...
//! CoreS3 — WiFi + BLE coexistence.
//!
//! Runs the WiFi STA bring-up plus a BLE peer-MAC scanner, listing discovered
//! MACs on the LCD under the IP. Coex must be built `--release` (the BLE deps
//! trip a dev-profile xtensa codegen bug):
...
use cores3::{Lcd, STRIP_BYTES, ble, connect_wifi, draw_demo, draw_status, init_display, init_i2c};
...
async fn main(spawner: embassy_executor::Spawner) {
    ...
    // Coex (WiFi + BLE) needs more controller heap. NOTE: esp-alloc's global heap
    // holds at most 3 regions — this internal heap, the reclaimed region above,
    // and the PSRAM region (below) are exactly 3, so do NOT add a 4th
    // `heap_allocator!`.
    ...
    draw_demo(&mut display, &mut strip_buf[..], "CoreS3", &["WiFi + BLE"]).await;
