...
use oxivgl::flush_pipeline::{DisplayOutput, FlushSync, SemaphoreFlushSync, UiError};
...
/// Counts what actually reaches the panel, by wrapping the display output.
///
/// A wrapper rather than counters inside `common::ui::driver`: the framework is
/// shared by every LVGL example and should not carry this harness's
/// instrumentation. `DisplayOutput` is a trait, so the harness can interpose
/// without the framework knowing.
pub struct Counted<D>(pub D);

impl<D: DisplayOutput> DisplayOutput for Counted<D> {
    async fn show_raw_data(
        ...
        let r = self.0.show_raw_data(x, y, w, h, data).await;
        FLUSH_BYTES.fetch_add(data.len() as u32, Relaxed);
